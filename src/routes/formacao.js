...
router.patch('/:formacaoId', async (req, res) => {
  const pessoaId = req.params.id;
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      return res.status(response.status).json({ message: 'Erro ao atualizar formação', detalhe: errorText });
    }
    ...
    res.status(500).json({ message: 'Erro ao atualizar formação', error: error.message });
  }
});
